...
from finetune_v4 import Fine_Tune
...
# torchrun --standalone --nnodes=1 --nproc_per_node=8 experiments/eval/eval_tune_v3.py
...
def tune_train(predict_model, train_loader, criterion, optimizer, gpu):
    ...
    for data in train_loader:
        ...
        output_diff = predict_model(images_diff, diff=True)

        # print(output.size(), label.size())

        # loss = criterion(output, label)
        loss = (criterion(output, label) + criterion(output_diff, label)) / 2
...
def main():
    ...
    tune_folder = tune_folder+'_lr%s_wd%s_mo%s_dr%s_bs%s_ds%s_sl%s_dim%s_linear%s_all_v3' % (args.lr, args.wd, args.momentum, args.dropout, args.batch_size, args.downsample, args.seq_len, args.dim, args.linear)
