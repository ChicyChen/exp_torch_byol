    ...
    for data in train_loader:
        ...
        if not args.sequential:
            for i in range(args.num_seq-1):
                ...
                total_loss += loss.mean().item() / (args.num_seq-1)
...
def main():
    ...
    ckpt_folder='/home/siyich/byol-pytorch/checkpoints_pc_na%s_bnl%s_pbnl%s_il%s_ns%s/mse%s_std%s_cov%s_predln%s_hid%s_prj%s_sym%s_closed%s_sequential%s_bs%s_lr%s_wd%s' \
        % (args.num_aug, args.bn_last, args.pred_bn_last, args.inter_len, args.num_seq, args.mse_l, args.std_l, args.cov_l, args.pred_layer, args.pred_hidden, args.projection, args.sym_loss, args.closed_loop, args.sequential, args.batch_size, args.lr, args.wd)

    if not os.path.exists(ckpt_folder):
        os.makedirs(ckpt_folder)

    logging.basicConfig(filename=os.path.join(ckpt_folder, 'pcvic_train.log'), level=logging.INFO)
