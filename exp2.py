...
def main():
    ...
    knn_list = range(args.start_epoch, args.epochs,10)
    lowest_loss = np.inf
    best_epoch = 0
    ...
    plt.xticks(epoch_list, knn_list)
    plt.legend()
    plt.savefig(os.path.join(
        ckpt_folder, 'epoch%s_bs%s_loss.png' % (args.epochs+1, args.batch_size)))

    plt.plot(knn_list, train_loss_list, label = 'train')
    if not args.no_val:
        plt.plot(knn_list, test_loss_list, label = 'val')
