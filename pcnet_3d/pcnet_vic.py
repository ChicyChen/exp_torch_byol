...
class PCNET_VIC(nn.Module):
    def __init__(
        ...
        predictor = 1, # 0: MLP; 1: Res_MLP; 2: ODE-not supported
        relu_last = False,
        num_predictor = 2,
    ):
        super().__init__()
        ...
        self.predictor_forward = create_pred(projection_size, projection_size, projection_hidden_size, pred_bn_last, relu_last) 
        if num_predictor == 2:
            self.predictor_reverse = create_pred(projection_size, projection_size, projection_hidden_size, pred_bn_last, relu_last) 
        else:
    ...
    def forward(
        ...
        # mode 0
        pred_z_forward = self.predictor_forward(gt_z_all[:, :-1, :].view(B*(N-1),-1)).reshape(B, N-1, -1)
        loss_one = self.loss_fn(pred_z_forward, gt_z_all[:, 1:, :]) 
        if self.closed_loop:
            closed_z_reverse = self.predictor_reverse(pred_z_forward).reshape(B, N-1, -1)
            loss_one += self.loss_fn(closed_z_reverse, gt_z_all[:, :-1, :])
            loss_one = loss_one / 2

        if self.sym_loss:
            pred_z_reverse = self.predictor_reverse(gt_z_all[:, 1:, :].view(B*(N-1),-1)).reshape(B, N-1, -1)
            loss_two = self.loss_fn(pred_z_reverse, gt_z_all[:, :-1, :])
            if self.closed_loop:
                closed_z_forward = self.predictor_forward(pred_z_reverse).reshape(B, N-1, -1)
