...
def main():
    ...
    if not args.hmdb:
        args.class_num = 101
        if args.random:
            logging.basicConfig(filename=os.path.join(ckpt_folder, 'ucf_ft_epoch0.log'), level=logging.INFO)
        else:
            logging.basicConfig(filename=os.path.join(ckpt_folder, 'ucf_ft_epoch%s.log' % args.epoch_num), level=logging.INFO)
    else:
        args.class_num = 51
        if args.random:
            logging.basicConfig(filename=os.path.join(ckpt_folder, 'hmdb_ft_epoch0.log'), level=logging.INFO)
        else:
            logging.basicConfig(filename=os.path.join(ckpt_folder, 'hmdb_ft_epoch%s.log' % args.epoch_num), level=logging.INFO)
